...
class PaymentsPendingSynchronizer(
    private val fetchPending: suspend () -> List<PaymentEntity>,
    private val enqueuer: PaymentsWorkEnqueuer
    ...
    override suspend fun sync(context: SyncContext): SyncResult = withContext(Dispatchers.IO) {
        ...
        capped.forEach { payment ->
            runCatching { enqueuer.enqueue(payment.ID, replace = false) }
    ...
    companion object {
        const val NAME: String = "PAYMENTS"
    }
}
