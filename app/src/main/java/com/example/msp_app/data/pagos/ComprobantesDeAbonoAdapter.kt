...
class ComprobantesDeAbonoAdapter(
     ...
     * que es lo que el multipart necesita — y cobranza acepta PDF a propósito,
     * porque los recibos SAT llegan así.
     */
    override suspend fun importar(uri: String): ComprobanteDelAbono {
        ...
        if (!Comprobantes.permitido(tipo)) {
            ...
            return ComprobanteDelAbono(id = id, archivo = crudo.absolutePath, mime = tipo)
        }
        if (tipo == PDF) {
            val destino = File(context.filesDir, "$PREFIJO_COMPROBANTE$id.pdf")
            crudo.copyTo(destino, overwrite = true)
            crudo.delete()
            return ComprobanteDelAbono(id = id, archivo = destino.absolutePath, mime = tipo)
        }
        val resultado = ImageCompressor.compressImage(
            context = context,
            uri = Uri.fromFile(crudo),
            outputFileName = "$PREFIJO_COMPROBANTE$id.jpg"
        )
        crudo.delete()
        return ComprobanteDelAbono(
            id = id,
     ...
     * medidas, [Comprobantes.muestreoPara] decide el `inSampleSize` y la segunda
     * decodifica ya reducido. Cargar el bitmap completo para pintarlo en 104dp es
     * lo que hace que una rejilla de cinco fotos reviente un teléfono de gama
     * baja.
     *
    ...
    override suspend fun miniatura(archivo: String): Miniatura? {
        ...
        val opciones = BitmapFactory.Options().apply {
            inSampleSize = Comprobantes.muestreoPara(medidas.outWidth, medidas.outHeight)
