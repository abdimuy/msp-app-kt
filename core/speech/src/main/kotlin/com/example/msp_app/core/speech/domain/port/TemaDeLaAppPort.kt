...
/**
 * Puerto hacia el tema **GLOBAL** de la app (`ThemeController`, `:app`), para la
 * pantalla de descarga del dictado.
 *
 * ## Por qué existe
 *
 ...
 * llamada directa: `ThemeController` vive en `:app`, que `:core:speech` no puede
 * importar. **El puerto se queda en el módulo y el adaptador se va a `:app`**
 * (Ruling BF, tercera cláusula: cruza módulo; precedentes `LiquidacionPort`,
 * `ReportThemePort` y el `TemaDeLaAppPort` de `:feature:pagos`).
 *
 ...
 * **cada módulo cruza la frontera hacia `:app` por su cuenta.** Aquí es todavía
 * más fuerte que entre dos features: `:core:speech` es un módulo `core`, y
 * hacerlo depender de `:feature:pagos` invertiría el sentido del grafo —
 * `:feature:pagos` es de los que dependen de `core`, no al revés.
 *
 ...
 * y necesitan saber cuál dibujar. La pantalla de descarga no lo pinta todavía:
 * instala el host —para que el mecanismo esté donde tiene que estar el día que
 * el glifo llegue— pero no tiene control que dependa del estado del tema. Un
 * miembro que nadie llama es un contrato que hay que mantener sin nada que lo
 * verifique; cuando la pantalla pinte el glifo, se agrega entonces.
 */
