...
import com.example.msp_app.core.testing.sync.ScriptedSyncHealthSource
...
class SyncHealthSourceTest {
    ...
    fun `emits the transition from BACKLOG to HEALTHY as items get acked`() = runTest {
        val source = ScriptedSyncHealthSource(
            emissions = listOf(
    ...
    fun `a single HEALTHY emission with nothing pending completes cleanly`() = runTest {
        val source =
            ScriptedSyncHealthSource(emissions = listOf(SyncHealth(pending = 0, confirmed = 0)))
    ...
    fun `an empty source completes without emitting`() = runTest {
        val source = ScriptedSyncHealthSource(emissions = emptyList())
