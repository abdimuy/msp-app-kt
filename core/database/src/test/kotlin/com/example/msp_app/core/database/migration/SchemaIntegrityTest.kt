...
/**
 * Guardia de drift de esquema sobre v30 (spec Plan 2 Task 4). `MigrationTestHelper`
 * lee `core/database/schemas/.../30.json` (el commiteado por `exportSchema`, ver
 ...
 * que introdujo el 27.json), así que este test SOLO puede validar v30 contra sí
 * mismo — no prueba que las migraciones existentes produzcan un esquema
 * idéntico al v30 real (eso lo cubren [PaymentSurvivalMigrationTest] y
 * [MigrationSmokeTest], que no dependen de JSONs históricos). Las excepciones
 * son la 27→28, la 28→29 y la 29→30: ya cuentan con el JSON de la versión
 * anterior, así que [Migration27to28Test], [Migration28to29Test] y
 * [Migration29to30Test] sí las validan de punta a punta con ese harness.
 */
