...
private const val MIGRATION_DB = "migration-30-31-test.db"
private const val OLD_VERSION = 30
private const val NEW_VERSION = 31
private const val SEEDED_SALE_ID = "sale-migracion-30-31"
...
/**
 * Migración 30→31 validada de punta a punta contra los JSON reales (mismo
 * patrón que [Migration29to30Test]): la base se crea desde el `30.json`
 ...
 * esquema resultante contra el `31.json`.
 *
 ...
 * app.
 */
class Migration30to31Test : RobolectricTestBase() {
     ...
     * que `MIGRATION_30_31` estuviera REGISTRADA en la configuración real de
     ...
     *
     * Esta prueba en cambio abre el archivo v30 sembrado por el MISMO camino
     * que usa producción: `AppDatabase.buildDatabase` (única fuente de verdad
     * del builder, ver su companion object). Si `MIGRATION_30_31` no está en
    ...
    fun `MIGRATION_30_31 esta registrada en la configuracion real que usa produccion`() {
        ...
        try {
            assertTrue(
                "abrir por el camino real de produccion debe pasar por v31 sin bloquearse " +
            ...
            assertEquals(
                "la venta sembrada en v30 debe sobrevivir a la apertura real",
    ...
    @Test
    fun `la venta pendiente y sus hijos sobreviven la migracion 30 a 31`() {
        seedPendingSaleWithChildren()
